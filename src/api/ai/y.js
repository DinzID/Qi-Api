...
const fileTypeModule = require('file-type'); // GANTI NAMA VARIABLE
...
module.exports = function(app) {
    ...
    // Endpoint: AI Image Editing - GET VERSION
    app.get('/ai/image/edit', async (req, res) => {
        try {
            ...
            const fileType = await fileTypeModule.fromBuffer(resultBuffer); // PERUBAHAN DI SINI
    ...
    // Endpoint: AI Edit dari URL - GET VERSION
    app.get('/ai/image/edit/url', async (req, res) => {
        try {
            ...
            const fileType = await fileTypeModule.fromBuffer(imageBuffer); // PERUBAHAN DI SINI
            ...
            // Process dengan AI
            const resultBuffer = await img2img(imageBuffer, prompt);
            const resultFileType = await fileTypeModule.fromBuffer(resultBuffer); // PERUBAHAN DI SINI
