...
module.exports = function(app) {
    ...
    app.get('/ai/image/edit', async (req, res) => {
        try {
            ...
            const fileType = await fileType.fromBuffer(resultBuffer); // PERUBAHAN DI SINI
    ...
    app.get('/ai/image/edit/url', async (req, res) => {
        try {
            ...
            const fileType = await fileType.fromBuffer(imageBuffer); // PERUBAHAN DI SINI
            ...
            // Process dengan AI
            const resultBuffer = await img2img(imageBuffer, prompt);
            const resultFileType = await fileType.fromBuffer(resultBuffer); // PERUBAHAN DI SINI
